...
#define PFC_U_IN_IIR_B0                     (0.16)
#define PFC_U_IN_IIR_B1                     (0.16)
#define PFC_U_IN_IIR_A1                     (0.68)
...
#define PFC_VIN_PHASEDETECT_TH                        60.0    /* Input voltage threshold for phase detection [V] */
...
#define PFC_I_HARDWARE_OFFSET                                   2700
#define PFC_I_HARDWARE_LIMIT(OC_A)                              ((uint32_t)((PFC_I_HARDWARE_OFFSET>>3)-(OC_A*4095.0/PFC_I_SCALE)))
/*************************************** Fault thresholds ************************************************/
...
#define PFC_UNDERVOLT_LIMIT              	300.0 	/* Min DC bus voltage [V] */
#define PFC_IOVER_LIMIT                     4.0 	/* Max input current [A] */
...
#define PFC_VIN_THRESHOLD_RELAY_ON          100.0    /* After DC bus reaches a certain value, relay is turned on when AC input voltage is below this value [V] */

/*************************************** Voltage(DC bus) control ************************************************/
#define PFC_U_DCB_REF					    380.0   /* Required DC bus voltage [V] */
#define PFC_U_SOFTSTART_STEP                0.05     /* DC bus reference change step in soft start [V] */
#define PFC_U_DCB_BURSTON                   (PFC_U_DCB_REF-3.0)   /* In light load mode, current control is enabled when DC bus voltage falls below this value */
...
//--------------------------------------------------------------------
#define PFC_U_DCB_IIR_B0                    (0.42)//fc = 2kHz
#define PFC_U_DCB_IIR_B1                    (0.42)
#define PFC_U_DCB_IIR_A1                    (0.16)

#define PFC_U_P_GAIN						(5.633)		/* Voltage controller proportional gain 7.633*/
#define PFC_U_I_GAIN						(0.056)	/* Voltage controller integral gain 0.096*/
#define PFC_U_UPPER_LIMIT   				(1.0)  	/* Voltage controller upper limit, used in initialization */
#define PFC_U_LOWER_LIMIT    				(0.0) 	/* Voltage controller lower limit, used in initialization */
#define LOOP_SCALE           				(0.0456) 			/* 1.414/PFC_I_SCALE */     
#define LOW_CURRENT          				(0.025)  			/* Lower rms limit of input current reference, [A] */
#define HIGH_CURRENT         				(4.0)   			/* Upper rms limit of input current reference, [A] */
#define PFC_U_ERROR_LIMIT               	(50.0)  /* Output voltage error limit */
...
//---------------------------------------------------------------------
#define PFC_I_P_GAIN1						(0.228) 		/* Current controller proportional gain 0.515 */
#define PFC_I_I_GAIN1						(0.031) 		/* Current controller integral gain */
#define PFC_I_UPPER_LIMIT   				(0.99) 	/* Current controller upper limit */
#define PFC_I_LOWER_LIMIT    				(-0.99)   /* Current controller lower limit */
#define PFC_DCM_DUTY_COEFF   				(1.152)    /*1.152 (2*L*PFC_I_SCALE)/(Ts*V_DCB_SCALE) */

/**************************************** Application time base ***********************************************/
#define PFC_PWM_FREQ                        48                 /* PFC switch frequency [kHz] */
#define PWM_FREQ_VS_CTR_FREQ                1                       /* PWM frequency vs fast control frequency */
#define PFC_SLOW_LOOP_FREQ                  5                      /* PFC slow control loop frequency [kHz] */
...
#define PFC_DC_BUS_FLUCTUATION               15.0   /* When DC bus voltage fluctuation is larger than this value, current reference will be compensated  */
...
#define PFC_DC_BUS_BURST_UNDER_DELTA              6.0  /* During burst mode, when DC bus voltage is lesser than (PFC_U_DCB_BURSTON-PFC_DC_BUS_BURST_UNDER_DELTA), go to normal sub-state */
